...
abstract class HtmlImport extends GlobalImport
{
     ...
     */
    protected function loadHtml($url=null)
    {
        if (is_null($url)) {
            $url = $this->source;
        }
        ...
        $content = shell_exec("curl -sS --compressed -H \"User-Agent: 'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/605.1.15 (KHTML, like Gecko) Chrome/91.0.4472.114 Safari/605.1.15'\" \"".$this->urlFullPath($url).'"');
